...
navLinksEl.addEventListener("click", (e) => {
  ...
  if (e.target.classList.contains("nav__link")) {
    //This is important because user might click little bit outside of the nav links which will nkt yeild sectionId hence no scrolling
...
tabbedConatinerEl.addEventListener("click", (e) => {
  ...
  currentActiveContent = document.querySelector(
    `.operations__content--${customDataValue}`
  );
...
const handleHover = function (e, opacity) {
  //If we use this keyword with bind the value of this is going to be the first argument ot can be objects or even values.
...
const shifter = function (entries, observer) {
  entries.forEach((ent) => {
    if (ent.isIntersecting) {
